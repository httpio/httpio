...
public class ProjectController implements ControllerInterface {
    ...
    private final String SCOPE_REQUEST = "StandardRequest";
    ...
    public void prepare() {
        ...
        requestMethod.setItems(FXCollections.observableList(Method.createAllMethodsList()));
    ...
    private void reloadResponse() {
        ...
        responseHeaders.setText(lastResponse.getRawHeaders());

        if (lastResponse.getBody() != null) {
            responseBody.setText(lastResponse.getBody().getContent());
            jsonTreeField.setInput(lastResponse.getBody().getContent());
        } else {
            responseBody.setText(null);
            jsonTreeField.setInput(null);
        }
    ...
    private void reloadRaw() {
        if (profile == null || request == null) {
            ...
            try {
                requestRawTextArea.setText((new RequestBridge(request, profile)).getRAW());
            } catch (MalformedURLException e) {
                requestRawTextArea.setText("Error: " + e.getMessage());
            }
        }
    }
